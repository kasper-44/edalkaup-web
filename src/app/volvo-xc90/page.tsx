...
import { xc90Cars, formatXC90Price, colorHex } from '@/data/xc90'
...
function StatusBadge({ status }: { status: 'available' | 'reserved' | 'sold' }) {
  const styles = {
    ...
    reserved:
      'bg-accent/15 text-accent-dark dark:text-accent border-accent/25',
...
export default function VolvoXC90Page() {
  return (
    <>
      {/* ─── Hero ─── */}
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-4 mb-8">
                <div className="bg-white dark:bg-navy-700/60 rounded-xl border border-black/5 dark:border-white/5 px-5 py-3 text-center">
                  <p className="text-2xl font-bold text-accent">455 hö</p>
                  <p className="text-xs text-gray-500 dark:text-slate-400">Samtals afl</p>
                </div>
                <div className="bg-white dark:bg-navy-700/60 rounded-xl border border-black/5 dark:border-white/5 px-5 py-3 text-center">
                  <p className="text-2xl font-bold text-accent">71 km</p>
                  <p className="text-xs text-gray-500 dark:text-slate-400">Rafmagnsakdrif</p>
                </div>
                <div className="bg-white dark:bg-navy-700/60 rounded-xl border border-black/5 dark:border-white/5 px-5 py-3 text-center">
      ...
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {xc90Cars.map((car) => (
            <article
              key={car.id}
              className={`group bg-white dark:bg-navy-800 rounded-2xl overflow-hidden border border-black/5 dark:border-white/5 hover:border-accent/20 transition-all duration-300 hover:shadow-2xl hover:shadow-accent/5 flex flex-col ${
                car.status === 'sold' ? 'opacity-60' : ''
              }`}
            >
              {/* Image */}
              ...
              <div className="p-5 flex flex-col flex-1">
                ...
                <p className="text-xs text-gray-400 dark:text-slate-500 mb-4">
                  Sjálfskipting · Raf.tengill / Bensín · Fjórhjóladrif · 7 sæti
                </p>
                ...
                {car.packages.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {car.packages.map((pkg) => (
                      <span
                        key={pkg}
                        className="text-[11px] px-2 py-0.5 rounded-md bg-gray-100 dark:bg-navy-700 text-gray-500 dark:text-slate-400 border border-black/5 dark:border-white/5"
                      >
                        {pkg}
                      </span>
                    ))}
        ...
        <div className="relative bg-gradient-to-r from-accent/10 to-accent/5 rounded-3xl border border-accent/20 p-10 sm:p-16 text-center overflow-hidden">
          ...
          <div className="relative">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              ...
              <a
                href="https://m.me/edalkaup"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-8 py-4 text-base font-semibold border border-accent/30 text-accent rounded-xl hover:bg-accent/10 transition-colors"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2C6.477 2 2 6.145 2 11.243c0 2.907 1.45 5.499 3.716 7.19V22l3.427-1.88A11.26 11.26 0 0012 20.485c5.523 0 10-4.144 10-9.242C22 6.145 17.523 2 12 2zm1.07 12.448l-2.545-2.714-4.97 2.714 5.467-5.8 2.609 2.714 4.905-2.714-5.466 5.8z" />
                </svg>
                Messenger
              </a>
            </div>
